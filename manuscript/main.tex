...
Agent Workflow Memory induces reusable textual workflows from past action
trajectories and selectively provides them to guide later generations
\cite{agentWorkflowMemory2025}. At the serving layer,
...
Sherlock combines within-run selective verification, background speculation,
and rollback for agentic workflows \cite{sherlock2025}. Speculative Actions
...
The first follows eight diff-cutoff events in 18 frozen agent runs. The second
measures substring or 3-gram Jaccard overlap on 4,968 registered non-system
block--agent incidences in 100 traces. The third restricts a 1,000-row
all-block sample to 763 document--consumer pairs and parses nested titles
before testing mention against repair flips. None instruments actual access.
...
\midrule
Safe equivalence of adjacent invocations & output diff and command similarity & All 8 events were parent-directory mtime artifacts; 6/8 responses, 4/8 next commands, 8/8 suffixes, and 4/8 exits differed. Complete next-invocation matches: 0. & The cutoff did not identify safe equivalence; this does not show exact reuse never occurs. \\
Per-agent use of a sentence region & substring and $0.5$-threshold Jaccard & On 4,968 registered incidences, zero-use was 98.09\% and 95.69\%; both medians and 90th percentiles were zero. & The measure was degenerate. Zero copied text is not evidence of no access or dependence. \\
Downstream use of a document & title mention in final output & Of 763 document pairs, 61 mentioned pairs had 0 repair flips; 702 unmentioned pairs had 36 (5.13\%), versus 35 (4.99\%) identity-control flips. & Mention was not a validated usage oracle or causal-direction test. \\
\bottomrule
\end{tabular}
...
The registered overlap population includes 1,092 retriever-side cite/document
incidences. Excluding them gives 3,876 downstream incidences with 97.70\% and
94.71\% zero-use; both median and 90th percentile remain zero.
...
nor sufficient for dependence, and its 5.13\% unmentioned-document repair-flip
rate was descriptively close to the 4.99\% identity-control rate on the same
702 pairs. Grouping them as ``proxy failures'' is useful for measurement design,
...
\cite{autogen2024,autogenRepo}, a LangGraph two-consumer fixture
\cite{langgraphRepo}, and SWE-agent RetryAgent's optional Preselector path
\cite{sweagent2024,sweagentRepo}. They span
...
For each audit unit we recorded evidence fields covering the complete consumer
...
coverage. For each audit unit, we cross-checked at least two load-bearing
locators spanning routing and consumer-input
...
The archived dataset starts from the 164-task Python test split of
HumanEvalPack \cite{octopack2023}, which supplies source and tests; our paired
archive additionally stores reconstructed pass/fail labels. The
...
the diff-analysis records, and the injection bundle require external files
listed by hash. The anonymous code/data supplement contains analysis paths,
fixtures, derived ledgers, and manifests. External hash-bound inputs remain
unavailable for independent aggregate reproduction from the submission alone.
...
another; qualification should precede execution and interpretation. Until
native traces bind these records, code paths and final-text diagnostics should
be reported as qualification evidence rather than realized reuse.
An identifiable evaluation should state the record supporting each claim and
leave unavailable quantities unobserved. This separation keeps negative
qualification results informative without converting them into a universal
claim that computation reuse is impossible.
