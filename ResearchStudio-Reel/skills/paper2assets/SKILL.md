...
1. Run `fetch_logos.py` (command above). It prints a **✓/✗ CHECKLIST** to stderr and a JSON object `{"logos": [...], "missing": ["Institute A", ...]}` to stdout, AND writes the same object to **`<outdir>/assets/logos/logos.json`**.
   - **ALWAYS take each logo's `path`/`slug` from this JSON — never invent a filename from the institute name.** The script canonicalizes + maps names (e.g. *Microsoft Research Asia* → slug `microsoft` → `assets/logos/microsoft.png`, mirroring Google Research → Google), so a hand-guessed `microsoft-research-asia.png` will be WRONG and point at a nonexistent/old file. If you piped stdout through `tail`/`head` and lost entries, read `assets/logos/logos.json` — it always has the full, correct list.
